...
def run_scenario_pipeline(payload: ScenarioPayload) -> ScenarioResult:
    ...
    try:
        mock_path = os.path.join(
            os.path.dirname(__file__), "..", "..", "mock_data", "gates.json"
        )
        with open(mock_path, "r") as f:
            gates = json.load(f)
        crowd_status = " | ".join(
            [
                f"{g['name']}: {g['currentCrowd']}/{g['capacity']} ({g['riskLevel']})"
                for g in gates
            ]
        )
    except Exception:
    ...
    task_prompt = (
        template.replace("{CROWD_STATUS}", crowd_status)
    ...
    for attempt in range(retries + 1):
        try:
            logger.info(
                f"Attempting Scenario Gemini Analysis (Attempt {attempt + 1}/{retries + 1})"
            )
            ...
            logger.info(
                f"Scenario Gemini Analysis Successful. Time: {elapsed:.2f}s, Confidence: {result.confidence}"
            )
            return result

        except Exception as e:
            logger.error(
                f"Scenario Gemini Analysis Failed (Attempt {attempt + 1}): {e}"
            )
            if attempt == retries:
                logger.warning(
                    "Max retries reached. Activating Scenario Fallback Rule Engine."
                )
                break
